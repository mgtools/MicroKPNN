...
# Write y true to file val
val_y_true_file = open(os.path.join(outPath, "tf_yTrue_val.csv"),"w")
val_y_true_file.write(",".join(outputs) + "\n")
val_y_true_file.write(pd.DataFrame(np.transpose(y_test.astype("int"))).to_csv(header=False, index=False))
val_y_true_file.close()
